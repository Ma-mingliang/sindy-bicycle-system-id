"""Quick Screen: History Window (3-step input).

Tests if adding history information improves prediction.
"""
import sys
import os
import json
...
ACTION_DIM = 1
HISTORY_STEPS = 3
INPUT_DIM = STATE_DIM * HISTORY_STEPS + ACTION_DIM  # 7*3 + 1 = 22
...
class HistoryWindowNet(nn.Module):
    """History window prediction: delta_s = f(s_{t-2}, s_{t-1}, s_t, a_t)."""
    def __init__(self, hidden=64, depth=3, activation='tanh', history_steps=3):
        super().__init__()
        self.history_steps = history_steps
        input_dim = STATE_DIM * history_steps + ACTION_DIM
    ...
    def forward(self, s_history, a):
        # s_history: (batch, history_steps, 7)
        # a: (batch, 1)
        x = torch.cat([s_history.reshape(s_history.shape[0], -1), a], dim=-1)
        return self.net(x)
...
def train_history_window(data, config, seed=43):
    """Train history window model."""
    ...
    dt = 1.0 / 30.0
    history_steps = config.get('history_steps', HISTORY_STEPS)
    ...
    print(f"Training History Window model (seed={seed}, history={history_steps})...")
    ...
    for epoch in range(config['n_epochs']):
        ...
        for _ in range(100):
            ...
            for _ in range(batch_size):
                ...
                ep = episodes[ep_idx]
                if ep['length'] < history_steps + 1:
                    continue
                idx = np.random.randint(history_steps, ep['length'] - 1)
                history_list.append(ep['obs'][idx-history_steps:idx])
            ...
            sb = torch.FloatTensor(np.array(history_list) / state_std)
            ab = torch.FloatTensor(np.array(actions_list).reshape(-1, 1) / action_std)
            target = torch.FloatTensor(np.array(deltas_list) / (delta_std * dt))
...
def evaluate(model, data, state_std, action_std, delta_std, horizons, n_segments=5, seed=42):
    ...
    for h in horizons:
        ...
        for seg in segments:
            ...
            history = [s0.copy()] * history_steps

            for step in range(n):
                try:
                    # Prepare history input
                    history_arr = np.array(history[-history_steps:])
                    s_norm = torch.FloatTensor(history_arr / state_std).unsqueeze(0)
...
def main():
    print("=" * 60)
    print("Quick Screen: History Window (3-step)")
    print("=" * 60)
    ...
    print("\n2. Training History Window model...")
    model, state_std, action_std, delta_std = train_history_window(data, config, seed=43)
    ...
    print("\n" + "=" * 60)
    print("RESULTS: History Window (3-step)")
    print("=" * 60)
    ...
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'History Win':<12} {'Improvement':<12}")
    print("-" * 46)
    v9_nmae = {1: 0.0051, 10: 0.0628, 50: 0.4557, 100: 0.5064, 200: 0.4737, 500: 0.5529}
    for h in horizons:
        v9_val = v9_nmae.get(h, float('nan'))
        hw_val = results[h]['nmae_mean']
        if v9_val > 0 and not np.isnan(hw_val):
            improvement = (v9_val - hw_val) / v9_val * 100
            print(f"H={h:<7} {v9_val:<12.4f} {hw_val:<12.4f} {improvement:<12.1f}%")

    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_72h',
        'experiment': 'history_window_3step',
        'config': config,
        'results': results,
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP005_history_window.json'
    with open(output_path, 'w') as f:
        json.dump(output, f, indent=2, default=str)
    print(f"\nResults saved to: {output_path}")

    model_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/07_models/history_window_seed43.pt'
    torch.save({
        'config': config,
        'state_std': state_std,
        'action_std': action_std,
