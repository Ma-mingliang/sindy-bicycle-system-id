"""Physics-Informed Correction OOD Testing.

Tests the best method (combined NN + physics correction) on OOD data.
"""
import sys
import json
import time
...
def main():
    print("=" * 60)
    print("Physics-Informed Correction OOD Testing")
    print("=" * 60)
    ...
    print("\n4. Generating OOD scenarios...")
    ood_scenarios = generate_ood_scenarios(data, data['state_std'])
    print(f"  Generated {len(ood_scenarios)} OOD scenarios")

    print("\n5. Evaluating on OOD scenarios...")
    horizons = [1, 10, 50, 100, 200, 500]
    ood_results = evaluate_ood(model, ood_scenarios, data['state_std'], data['action_std'], data['delta_std'],
    ...
    output = {
        ...
        'experiment': 'physics_correction_ood',
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP035_physics_correction_ood.json'
