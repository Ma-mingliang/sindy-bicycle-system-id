"""Adaptive State Space Regularization.

Builds on breakthrough: reduce variance and improve H=500.
Key ideas:
1. State-dependent correction (more correction for sensitive states)
2. Adaptive correction strength based on distance from training distribution
3. Ensemble of corrected models
"""
import sys
import json
import time
...
# State-dependent correction weights (more correction for sensitive states)
STATE_CORRECTION_WEIGHTS = {
    'e_y': 1.5,      # Most sensitive
    'e_psi': 1.5,    # Most sensitive
    'v': 0.5,        # Less sensitive
    'theta': 1.0,    # Medium
...
def build_state_corrector(train_obs, state_std, n_neighbors=10):
    """Build a nearest-neighbor corrector for state-space regularization."""
...
def correct_state_adaptive(s_pred, nn_model, train_obs_norm, state_std, base_strength=0.1):
    """Adaptive state correction with state-dependent weights."""
    s_pred_norm = s_pred / state_std

    # Find nearest neighbors
    distances, indices = nn_model.kneighbors([s_pred_norm])

    # Compute distance-based adaptive strength
    mean_distance = np.mean(distances[0])
    adaptive_factor = min(1.0, mean_distance / 0.5)  # Normalize by typical distance
    adaptive_strength = base_strength * (1.0 + adaptive_factor)

    # Compute correction with state-dependent weights
    neighbor_mean = np.mean(train_obs_norm[indices[0]], axis=0)
    correction = (neighbor_mean - s_pred_norm) * adaptive_strength

    # Apply state-dependent weights
    weights = np.array([STATE_CORRECTION_WEIGHTS[name] for name in STATE_NAMES_7D])
    correction = correction * weights

    # Apply correction
    s_corrected_norm = s_pred_norm + correction
...
def evaluate_with_correction(model, data, state_std, action_std, delta_std, nn_model, train_obs_norm,
                              horizons, base_strength=0.1, n_segments=5, seed=42):
    """Evaluate model with adaptive state-space correction."""
    dt = 1.0 / 30.0
    episodes = data['episodes']
        ...
        for seg in segments:
            ...
            for step in range(n):
                try:
                    ...
                    # Apply adaptive state-space correction
                    s_next = correct_state_adaptive(s_next, nn_model, train_obs_norm, state_std, base_strength)
...
def main():
    print("=" * 60)
    print("Adaptive State Space Regularization")
    print("=" * 60)
    ...
    # Test different base strengths
    base_strengths = [0.05, 0.1, 0.15, 0.2]
    ...
    nn_model, train_obs_norm = build_state_corrector(data['train_obs'], state_std)
    ...
    for strength in base_strengths:
        print(f"\n{'='*60}")
        print(f"Testing base_strength={strength}")
        print(f"{'='*60}")

        results = evaluate_with_correction(
            model, data, state_std, action_std, delta_std,
            nn_model, train_obs_norm, horizons, base_strength=strength
        )
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: Adaptive State Space Regularization Results")
    print("=" * 60)

    print(f"\n{'Strength':<12} {'H=50':<10} {'H=100':<10} {'H=200':<10} {'H=500':<10} {'Primary':<10}")
    print("-" * 62)

    for strength, data_dict in all_results.items():
        r = data_dict['results']
        primary = data_dict['primary']
        print(f"{strength:<12} {r[50]['nmae_mean']:<10.4f} {r[100]['nmae_mean']:<10.4f} "
              f"{r[200]['nmae_mean']:<10.4f} {r[500]['nmae_mean']:<10.4f} {primary:<10.4f}")

    # Find best configuration
    best_strength = min(all_results.keys(), key=lambda k: all_results[k]['primary'])
    print(f"\nBest base strength: {best_strength} (Primary = {all_results[best_strength]['primary']:.4f})")

    # Compare with v9 baseline
    print(f"\nComparison with v9 baseline:")
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Best Adaptive':<12} {'Improvement':<12}")
    print("-" * 46)
    v9_nmae = {1: 0.0051, 10: 0.0628, 50: 0.4557, 100: 0.5064, 200: 0.4737, 500: 0.5529}
    best_results = all_results[best_strength]['results']
    for h in horizons:
        v9_val = v9_nmae.get(h, float('nan'))
        a_val = best_results[h]['nmae_mean']
        if v9_val > 0 and not np.isnan(a_val):
            improvement = (v9_val - a_val) / v9_val * 100
            print(f"H={h:<7} {v9_val:<12.4f} {a_val:<12.4f} {improvement:<12.1f}%")

    # Save results
    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_144h',
        'experiment': 'adaptive_state_regularization',
        'base_strengths': base_strengths,
        'results': {str(k): {'primary': v['primary']} for k, v in all_results.items()},
        'best_strength': best_strength,
        'state_correction_weights': STATE_CORRECTION_WEIGHTS,
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP030_adaptive_state_regularization.json'
