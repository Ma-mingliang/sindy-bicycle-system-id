"""MBPO-SAC v7: SINDy+NN World Model with short horizon rollouts.

Key improvement over v6: Replaces linear world model with SINDy+NN residual,
achieving 35x better single-step accuracy and 15-86x better 10-step accuracy.

Changes from v6:
1. SINDyNNWorldModel: full polynomial library + NN residual (replaces linear model)
2. Virtual rollout horizon: 8 -> 10 (within accuracy window)
3. Warmup/ramp: 200/200 -> 150/150 (faster SAC activation)

Control: tau = u_lqr + w * alpha * sac_action
"""
...
# ============================================================
# SINDy + NN Residual World Model (v7 core improvement)
# ============================================================

class ResidualNet(nn.Module):
    """NN that learns residual = true_delta - sindy_delta (in normalized space)."""
...
class SINDyNNWorldModel:
    """SINDy polynomial library + NN residual world model.

    Replaces the linear model (A_d @ s + B_d * tau) from v6 with:
        s_next = s + (delta_sindy + delta_nn) * state_std

    where delta_sindy = polynomial_library(s_norm, a_norm) @ Xi
      and delta_nn = ResidualNet(s_norm, a_norm)
    """

    def __init__(self, sindy_npz_path, nn_weights_path, K_lqr, device):
        self.K_lqr = K_lqr
        self.device = device

        # Load SINDy model
        sindy_data = np.load(sindy_npz_path)
        self.Xi = sindy_data['coefficients']      # (21, 4)
        self.state_std = sindy_data['state_std']   # (4,)
        self.action_std = float(sindy_data['action_std'])

        # Load NN residual
        self.res_net = ResidualNet().to(device)
        self.res_net.load_state_dict(torch.load(nn_weights_path, map_location=device))
        self.res_net.eval()

        print(f"  SINDy+NN World Model loaded:")
        print(f"    SINDy coefficients: {self.Xi.shape}, {np.sum(np.abs(self.Xi) > 1e-6)} nonzero terms")
        print(f"    state_std: {self.state_std}")
        print(f"    action_std: {self.action_std}")
        print(f"    NN params: {sum(p.numel() for p in self.res_net.parameters())}")

    def _build_library(self, s_norm, a_norm):
        """Build polynomial library up to degree 2.

        Args:
            s_norm: (4,) normalized state
            a_norm: scalar normalized action
        Returns:
            lib: (1, 21) library matrix
        """
        x = np.concatenate([s_norm, [a_norm]])  # (5,)
        n_feat = len(x)
        ...
        return np.array(features).reshape(1, -1)  # (1, 21)

    def predict_next(self, theta, delta, theta_dot, delta_dot, action, target_theta):
        """Predict next state using SINDy + NN residual.

        Args:
            theta, delta, theta_dot, delta_dot: physical state components
            action: SAC action (raw, will be scaled by ALPHA)
            target_theta: LQR target angle
        Returns:
            theta_next, delta_next, theta_dot_next, delta_dot_next
        """
        # LQR + SAC residual torque
        ...
        # SINDy prediction (in normalized delta space)
        lib = self._build_library(s_norm, a_norm)
        delta_sindy = (lib @ self.Xi)[0]  # (4,)

        # NN residual prediction (in normalized delta space)
        with torch.no_grad():
            s_t = torch.FloatTensor(s_norm).unsqueeze(0).to(self.device)
            a_t = torch.FloatTensor([a_norm]).unsqueeze(0).to(self.device)
            delta_nn = self.res_net(s_t, a_t).cpu().numpy()[0]  # (4,)

        # Combine and denormalize
        delta_combined = (delta_sindy + delta_nn) * self.state_std
        s_next = s + delta_combined
...
def sindy_nn_virtual_rollout(model, buffer, agent, n_rollouts=10, horizon=10):
    """Virtual rollout using SINDy+NN world model.

    Key improvement over v6: Uses accurate SINDy+NN model instead of linear.
    Horizon increased from 8 to 10 steps (within accuracy window).
    """
    data = []
...
def save_training_report(output_dir, config, baseline_stats, training_log, final_eval):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("强化学习训练完整报告 - 第一阶段：MBPO-SAC SINDy+NN世界模型\n")
        f.write("=" * 80 + "\n\n")
...
def train():
    ...
    print("=" * 70)
    print("MBPO-SAC v7: SINDy+NN World Model (35x more accurate)")
    print("=" * 70)
    ...
    # Load SINDy+NN world model (v7 core change)
    base_dir = 'D:/系统辨识作业/sindy_bicycle'
    sindy_nn_model = SINDyNNWorldModel(
        sindy_npz_path=os.path.join(base_dir, 'sindy_full_model.npz'),
        nn_weights_path=os.path.join(base_dir, 'sindy_nn_residual.pt'),
        K_lqr=K_lqr,
        device=device,
    )
    ...
    num_episodes = 600
    batch_size = 256
    warmup_episodes = 150   # v7: faster than v6 (200)
    ramp_episodes = 150     # v7: faster than v6 (200)
    rollout_horizon = 10    # v7: 10 steps (was 8 in v6)

    config = {
        'algorithm': 'MBPO-SAC v7 (SINDy+NN)',
        'world_model': 'SINDy polynomial + NN residual',
        'world_model_accuracy': '35x better than linear (v6)',
    ...
    # ============================================================
    eval_sigma_schedule = [int(150 + i * 8) for i in range(50)]
    ...
    print(f"World model: SINDy+NN (rollout horizon={rollout_horizon})")
    print(f"{'='*70}")
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            # Virtual rollouts with SINDy+NN model
            if steps % 100 == 0 and len(real_buffer) >= batch_size and env.sac_weight > 0:
                vdata = sindy_nn_virtual_rollout(sindy_nn_model, real_buffer, sac,
                                                  n_rollouts=10, horizon=rollout_horizon)
