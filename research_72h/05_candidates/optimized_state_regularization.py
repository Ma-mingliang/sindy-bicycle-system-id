"""Optimized State Space Regularization.

Builds on breakthrough: correction_strength=0.1 achieves 17.7% improvement.
Now optimizing for H=500 improvement.
"""
import sys
import json
import time
...
def main():
    print("=" * 60)
    print("Optimized State Space Regularization - Multi-Seed")
    print("=" * 60)
    ...
    for seed in seeds:
        ...
        model, state_std, action_std, delta_std = train_model(data, config, seed=seed)

        print(f"\nBuilding state corrector...")
        nn_model, train_obs_norm = build_state_corrector(data['train_obs'], state_std)

        print(f"\nEvaluating with correction_strength={correction_strength}...")
    ...
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Mean':<12} {'Std':<12} {'Improvement':<12}")
    print("-" * 58)
    v9_nmae = {1: 0.0051, 10: 0.0628, 50: 0.4557, 100: 0.5064, 200: 0.4737, 500: 0.5529}
    for h in horizons:
        v9_val = v9_nmae.get(h, float('nan'))
        h_vals = [all_results[seed]['results'][h]['nmae_mean'] for seed in seeds]
        h_mean = np.mean(h_vals)
        h_std = np.std(h_vals)
        if v9_val > 0 and not np.isnan(h_mean):
            improvement = (v9_val - h_mean) / v9_val * 100
            print(f"H={h:<7} {v9_val:<12.4f} {h_mean:<12.4f} {h_std:<12.4f} {improvement:<12.1f}%")

    # Save results
    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_144h',
        'experiment': 'optimized_state_regularization_multi_seed',
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP029_optimized_state_regularization.json'
