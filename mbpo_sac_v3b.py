"""MBPO-SAC v3b: LQR base + process noise + larger residual range.

Key improvements over v2:
1. Process noise (model uncertainty)
2. Actuator delay simulation
3. Larger ALPHA=0.3 for bigger residual range
4. 1200 episodes for longer training
5. Reward shaping: add smoothness penalty
"""
...
# ============================================================
# Environment with LQR + process noise
# ============================================================
...
class NoisyLQREnv:
    """Environment with LQR + process noise + actuator delay.

    Improvements over v2:
    1. Process noise (model uncertainty)
    2. Actuator delay (1-step delay)
    3. Larger ALPHA for bigger residual range
    """
    ...
    def __init__(self, K_lqr, v0=3.5, dt=1/30, max_steps=1000):
        ...
        # Process noise scale (increases with disturbance)
        self.base_noise = 0.0005
    ...
    def step(self, action):
        ...
        # Actuator delay: use previous action + current action blend
        ...
        # Dynamics with process noise (noise proportional to disturbance)
        dt_sub = self.dt / self.sub_steps
        A_m, B_m = ab_matrix(self.M, self.C1, self.K0, self.K2,
                             max(self.v0, 0.5), self.g)
        B_s = B_m[:, 1:2]

        # Noise scales with sigma (more noise at higher disturbance)
...
def train():
    print("=" * 70)
    print("MBPO-SAC v3b: LQR + process noise + actuator delay")
    print("=" * 70)
    ...
    env = NoisyLQREnv(K_true, v0=v0, max_steps=1000)
    ...
    print(f"Process noise: {env.base_noise} (scales with sigma)")
    print(f"Actuator delay: 0.7*current + 0.3*previous")
    ...
    }, 'D:/系统辨识作业/sindy_bicycle/mbpo_sac_v3b.pt')
    print("\nModel saved to mbpo_sac_v3b.pt")

    # Evaluation
    print(f"\n{'='*70}")
    print("Evaluation: LQR vs LQR+MBPO-SAC v3b")
    print(f"{'='*70}")
