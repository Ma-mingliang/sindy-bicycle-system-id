"""Multi-Seed Validation for Best Methods.

Validates the best methods (constrained Neural ODE, uncertainty gating) with multiple seeds.
"""
import sys
import json
import time
...
class ConstrainedODEFunc(nn.Module):
    ...
    def __init__(self, hidden=64, depth=3, activation='tanh', max_jacobian_norm=0.1):
        super().__init__()
...
def train_constrained_neural_ode(data, config, seed=42):
    ...
    model = ConstrainedODEFunc(
        ...
        max_jacobian_norm=config.get('max_jacobian_norm', 0.1)
    )

    opt = torch.optim.Adam(model.parameters(), lr=config['lr'])
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=config['n_epochs'])

    lambda_jacobian = config.get('lambda_jacobian', 1.0)
    ...
    for epoch in range(config['n_epochs']):
        ...
        for sb, ab, yb in loader:
            ...
            if lambda_jacobian > 0:
                ...
                loss_jacobian = torch.relu(jac_norm - config.get('max_jacobian_norm', 0.1))
...
def main():
    print("=" * 60)
    print("Multi-Seed Validation for Constrained Neural ODE")
    print("=" * 60)
    ...
    for seed in seeds:
        ...
        model, state_std, action_std, delta_std = train_constrained_neural_ode(data, config, seed=seed)
    ...
    output = {
        ...
        'experiment': 'multi_seed_constrained_neural_ode',
        'seeds': seeds,
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP018_multi_seed_constrained.json'
