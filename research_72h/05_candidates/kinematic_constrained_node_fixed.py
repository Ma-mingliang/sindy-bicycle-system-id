"""Kinematic-Constrained Neural ODE - FIXED VERSION.

Critical fix: Compute kinematics in PHYSICAL space, not normalized space!

Previous version had a bug: computed kinematics in normalized space,
which is incorrect because the kinematic formulas are exact in physical units.

Fix: Denormalize → compute kinematics → renormalize
"""
import sys
import json
import time
...
class KinematicConstrainedODEFunc(nn.Module):
    """Neural ODE with kinematic constraints for e_y and e_psi.

    CRITICAL FIX: Compute kinematics in PHYSICAL space!

    Architecture:
    - e_y, e_psi: Kinematic formulas (denormalize → compute → renormalize)
    - v, theta, theta_dot, delta, delta_dot: Neural ODE
    """
    def __init__(self, hidden=256, depth=5, activation='silu'):
        super().__init__()

        # Neural ODE for non-kinematic states
        ...
        layers.append(nn.Linear(hidden, 5))  # Only 5 non-kinematic states
    ...
    def forward(self, s, a):
        """Predict delta for all 7 states.

        Args:
            s: (batch, 7) - normalized states
            a: (batch, 1) - normalized actions

        Returns:
            delta: (batch, 7) - predicted deltas (normalized)
        """
        # Neural ODE for non-kinematic states
        x = torch.cat([s, a], dim=-1)
        nn_delta = self.net(x)  # (batch, 5)

        # Construct full delta
        batch_size = s.shape[0]
        delta = torch.zeros(batch_size, 7, device=s.device)

        # e_y and e_psi: Will be handled by kinematic constraints in evaluation
        delta[:, 0] = 0.0  # e_y (placeholder)
        delta[:, 1] = 0.0  # e_psi (placeholder)

        # v, theta, theta_dot, delta, delta_dot: Neural ODE
        delta[:, 2] = nn_delta[:, 0]  # v
        delta[:, 3] = nn_delta[:, 1]  # theta
        delta[:, 4] = nn_delta[:, 2]  # theta_dot
        delta[:, 5] = nn_delta[:, 3]  # delta
        delta[:, 6] = nn_delta[:, 4]  # delta_dot
...
def train_kinematic_constrained_fixed(data, config, seed=42):
    """Train kinematic-constrained Neural ODE with FIXED kinematics."""
    ...
    # FIX: Compute kinematic deltas in PHYSICAL space
    # e_y_dot = v * sin(e_psi) — physical units
    v_phys = train_obs[:, 2]  # Already in physical units (m/s)
    e_psi_phys = train_obs[:, 1]  # Already in physical units (rad)
    e_y_dot = v_phys * np.sin(e_psi_phys)  # m/s
    e_y_kinematic = e_y_dot * DT / delta_std[0]  # Normalize

    # e_psi_dot = -v * delta / L — physical units
    delta_phys = train_obs[:, 6]  # Already in physical units (rad)
    e_psi_dot = -v_phys * delta_phys / WHEELBASE  # rad/s
    e_psi_kinematic = e_psi_dot * DT / delta_std[1]  # Normalize

    # Replace e_y and e_psi deltas with kinematic values
    Y[:, 0] = e_y_kinematic
    Y[:, 1] = e_psi_kinematic
    ...
    model = KinematicConstrainedODEFunc(
    ...
    print(f"Training FIXED Kinematic-Constrained NODE (seed={seed})...")
...
def evaluate_kinematic_fixed(model, data, state_std, action_std, delta_std, horizons, n_segments=5, seed=42):
    """Evaluate kinematic-constrained model with FIXED kinematics."""
    dt = 1.0 / 30.0
        ...
        for seg in segments:
            ...
            for step in range(n):
                try:
                    ...
                    # FIX: Apply kinematic constraints in PHYSICAL space
                    # Denormalize current state
                    v_phys = s_cur[2] * state_std[2]
                    e_psi_phys = s_cur[1] * state_std[1]
                    delta_phys = s_cur[6] * state_std[6]

                    # Compute kinematics in physical space
                    e_y_dot = v_phys * np.sin(e_psi_phys)  # m/s
                    e_psi_dot = -v_phys * delta_phys / WHEELBASE  # rad/s

                    # Normalize and replace
                    dsdt_norm[0] = e_y_dot * dt / delta_std[0]
                    dsdt_norm[1] = e_psi_dot * dt / delta_std[1]
...
def main():
    print("=" * 60)
    print("FIXED Kinematic-Constrained Neural ODE")
    print("=" * 60)
    print("\nCRITICAL FIX: Compute kinematics in PHYSICAL space!")
    ...
    print("\n2. Training FIXED Kinematic-Constrained NODE...")
    model, state_std, action_std, delta_std = train_kinematic_constrained_fixed(data, config, seed=42)

    print("\n3. Evaluating...")
    horizons = [1, 10, 50, 100, 200, 500, 1000]
    results = evaluate_kinematic_fixed(model, data, state_std, action_std, delta_std, horizons)

    print("\n" + "=" * 60)
    print("RESULTS: FIXED Kinematic-Constrained NODE")
    print("=" * 60)
    ...
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Wide SiLU':<12} {'Fixed KC':<12} {'Improvement':<12}")
    print("-" * 58)
    v9_nmae = {1: 0.0051, 10: 0.0628, 50: 0.4557, 100: 0.5064, 200: 0.4737, 500: 0.5529, 1000: 0.6443}
    wide_silu = {1: 0.0155, 10: 0.0288, 50: 0.0708, 100: 0.3114, 200: 0.8716, 500: 1.2099, 1000: 1.3005}
    for h in horizons:
        v9_val = v9_nmae.get(h, float('nan'))
        ws_val = wide_silu.get(h, float('nan'))
        fkc_val = results[h]['nmae_mean']
        if v9_val > 0 and not np.isnan(fkc_val):
            improvement = (v9_val - fkc_val) / v9_val * 100
            print(f"H={h:<7} {v9_val:<12.4f} {ws_val:<12.4f} {fkc_val:<12.4f} {improvement:<12.1f}%")
    ...
    output = {
        ...
        'experiment': 'kinematic_constrained_node_fixed',
        'fix': 'Compute kinematics in PHYSICAL space, not normalized space',
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP054_kinematic_fixed.json'
