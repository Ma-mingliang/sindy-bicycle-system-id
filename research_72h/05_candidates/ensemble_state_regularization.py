"""Ensemble State Space Regularization.

Combines breakthrough state space regularization with ensemble methods.
Key idea: Multiple models + state space correction = better H=500.
"""
import sys
import json
import time
...
def correct_state(s_pred, nn_model, train_obs_norm, state_std, correction_strength=0.1):
    """Correct predicted state using nearest neighbors in training distribution."""
    ...
    correction = (neighbor_mean - s_pred_norm) * correction_strength
...
def evaluate_ensemble_with_correction(models, data, state_std, action_std, delta_std, nn_model, train_obs_norm,
                                       horizons, correction_strength=0.1, n_segments=5, seed=42):
    """Evaluate ensemble of models with state-space correction."""
    dt = 1.0 / 30.0
    episodes = data['episodes']
        ...
        for seg in segments:
            ...
            for step in range(n):
                try:
                    ...
                    # Apply state-space correction
                    s_next = correct_state(s_next, nn_model, train_obs_norm, state_std, correction_strength)
...
def main():
    print("=" * 60)
    print("Ensemble State Space Regularization")
    print("=" * 60)
    ...
    # Best correction strength from previous experiment
    ...
    for n_models in ensemble_sizes:
        ...
        print(f"\nEvaluating ensemble with correction_strength={correction_strength}...")
        results = evaluate_ensemble_with_correction(
            models, data, data['state_std'], data['action_std'], data['delta_std'],
            nn_model, train_obs_norm, horizons, correction_strength=correction_strength
        )

        print(f"\nResults for ensemble_size={n_models}:")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: Ensemble State Space Regularization Results")
    print("=" * 60)

    print(f"\n{'Ensemble':<12} {'H=50':<10} {'H=100':<10} {'H=200':<10} {'H=500':<10} {'Primary':<10}")
    print("-" * 62)

    for n_models, data_dict in all_results.items():
        r = data_dict['results']
        primary = data_dict['primary']
        print(f"{n_models:<12} {r[50]['nmae_mean']:<10.4f} {r[100]['nmae_mean']:<10.4f} "
              f"{r[200]['nmae_mean']:<10.4f} {r[500]['nmae_mean']:<10.4f} {primary:<10.4f}")

    # Find best configuration
    best_ensemble = min(all_results.keys(), key=lambda k: all_results[k]['primary'])
    print(f"\nBest ensemble size: {best_ensemble} (Primary = {all_results[best_ensemble]['primary']:.4f})")

    # Compare with v9 baseline
    print(f"\nComparison with v9 baseline:")
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Best Ensemble':<12} {'Improvement':<12}")
    print("-" * 46)
    v9_nmae = {1: 0.0051, 10: 0.0628, 50: 0.4557, 100: 0.5064, 200: 0.4737, 500: 0.5529}
    best_results = all_results[best_ensemble]['results']
    for h in horizons:
        v9_val = v9_nmae.get(h, float('nan'))
        e_val = best_results[h]['nmae_mean']
    ...
    output = {
        ...
        'experiment': 'ensemble_state_regularization',
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP031_ensemble_state_regularization.json'
