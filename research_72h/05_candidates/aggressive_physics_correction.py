"""Aggressive Physics-Informed Correction.

Target: 75%+ improvement across ALL horizons.
Key ideas:
1. Much stronger correction (correction_strength=0.3-0.5)
2. Higher physics weight (0.9-1.0)
3. State-dependent correction with much higher weights for e_y, e_psi
4. Adaptive correction that increases with horizon
"""
import sys
import json
import time
...
# Aggressive state-dependent correction weights
STATE_WEIGHTS = {
    'e_y': 3.0,      # Most sensitive - very high correction
    'e_psi': 3.0,    # Most sensitive - very high correction
    'v': 0.3,        # Less sensitive - very low correction
    'theta': 1.5,    # Medium-high
    'theta_dot': 1.5, # Medium-high
    'delta': 1.5,    # Medium-high
    'delta_dot': 1.5, # Medium-high
}
...
def correct_state_aggressive(s_pred, s_cur, nn_model, train_obs_norm, state_std, dt,
                              correction_strength=0.3, physics_weight=0.9, step=0):
    """Aggressive state correction with adaptive strength."""
    s_pred_norm = s_pred / state_std

    # Find nearest neighbors
    distances, indices = nn_model.kneighbors([s_pred_norm])

    # Adaptive correction: increase with step
    adaptive_factor = min(2.0, 1.0 + step / 100.0)  # Increases from 1.0 to 2.0
    effective_strength = correction_strength * adaptive_factor

    # Compute correction: move toward nearest neighbor mean
    neighbor_mean = np.mean(train_obs_norm[indices[0]], axis=0)
    nn_correction = (neighbor_mean - s_pred_norm) * effective_strength
...
def evaluate_aggressive(model, data, state_std, action_std, delta_std, nn_model, train_obs_norm,
                         horizons, correction_strength=0.3, physics_weight=0.9, n_segments=5, seed=42):
    """Evaluate model with aggressive correction."""
    dt = 1.0 / 30.0
    episodes = data['episodes']
        ...
        for seg in segments:
            ...
            for step in range(n):
                try:
                    ...
                    s_next = correct_state_aggressive(s_next, s_cur, nn_model, train_obs_norm, state_std, dt,
                                                      correction_strength, physics_weight, step)
...
def main():
    print("=" * 60)
    print("Aggressive Physics-Informed Correction")
    ...
    param_combinations = [
        {'correction_strength': 0.3, 'physics_weight': 0.9},
        {'correction_strength': 0.3, 'physics_weight': 1.0},
        {'correction_strength': 0.4, 'physics_weight': 0.9},
        {'correction_strength': 0.4, 'physics_weight': 1.0},
        {'correction_strength': 0.5, 'physics_weight': 0.9},
        {'correction_strength': 0.5, 'physics_weight': 1.0},
    ]
    ...
    for params in param_combinations:
        ...
        # Check if 75% improvement achieved
        v9_primary = 0.5110
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: Aggressive Physics Correction Results")
    print("=" * 60)

    print(f"\n{'Config':<15} {'H=50':<10} {'H=100':<10} {'H=200':<10} {'H=500':<10} {'Primary':<10} {'Improve':<10}")
    print("-" * 75)
    ...
    for name, data_dict in all_results.items():
        ...
        print(f"{name:<15} {r[50]['nmae_mean']:<10.4f} {r[100]['nmae_mean']:<10.4f} "
              f"{r[200]['nmae_mean']:<10.4f} {r[500]['nmae_mean']:<10.4f} {primary:<10.4f} {improvement:<10.1f}%")

    # Find best configuration
    best_config = min(all_results.keys(), key=lambda k: all_results[k]['primary'])
    best_improvement = all_results[best_config]['improvement']
    print(f"\nBest configuration: {best_config}")
    print(f"  Primary: {all_results[best_config]['primary']:.4f}")
    ...
    output = {
        ...
        'experiment': 'aggressive_physics_correction',
        'target_improvement': 75,
        'param_combinations': param_combinations,
        'results': {k: {'primary': v['primary'], 'improvement': v['improvement'], 'params': v['params']}
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP039_aggressive_physics_correction.json'
