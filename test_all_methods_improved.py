...
# 训练方法2：多步损失训练
# ============================================================
def train_multistep(baseline_cls, baseline_kwargs=None, n_epochs=100,
...
def multistep_predict(baseline, nn_model, ood_detector, s, tau,
...
# 测试函数
# ============================================================
def evaluate(baseline, nn_model, ood_det, norms, predict_fn, **kwargs):
    ...
    for seg_i in range(n_segments):
        ...
        for step in range(500):
            ...
            s_hybrid = predict_fn(baseline, nn_model, ood_det, s_hybrid, tau,
                                  state_std, action_std, delta_std, **kwargs)
            if abs(s_real[0]) > math.pi / 3:
                break
...
def print_results(name, baseline_errors, all_errors):
    eval_steps = [1, 5, 10, 20, 50, 100, 200, 500]
    print(f"\n  {'步数':>6} | {'纯基线':>10} | {name:>10} | {'改进倍率':>10}")
    print(f"  {'-'*50}")
...
if __name__ == '__main__':
    ...
    # 测试1：NeuralODE + 自适应残差缩放
    print(f"\n{'='*60}")
    print("NeuralODE + 自适应残差缩放")
    print(f"{'='*60}")
    baseline, nn_model, ood_det, norms = train_adaptive(NeuralODEMethod, max_scale=0.5)
    b_err, h_err = evaluate(baseline, nn_model, ood_det, norms, adaptive_predict)
    mae = print_results("自适应", b_err, h_err)
    results["NeuralODE+自适应"] = mae

    # 测试2：NeuralODE + 多步损失
    print(f"\n{'='*60}")
    print("NeuralODE + 多步损失(10步)")
    print(f"{'='*60}")
    baseline, nn_model, ood_det, norms = train_multistep(NeuralODEMethod, multistep_length=10, multistep_weight=0.5)
    b_err, h_err = evaluate(baseline, nn_model, ood_det, norms, multistep_predict, residual_scale=0.3)
    mae = print_results("多步损失", b_err, h_err)
    results["NeuralODE+多步损失"] = mae

    # 测试3：ParamID + 自适应残差缩放
    print(f"\n{'='*60}")
    print("ParamID + 自适应残差缩放")
    print(f"{'='*60}")
    baseline, nn_model, ood_det, norms = train_adaptive(ParamIDMethod, max_scale=0.5)
    b_err, h_err = evaluate(baseline, nn_model, ood_det, norms, adaptive_predict)
    mae = print_results("自适应", b_err, h_err)
    results["ParamID+自适应"] = mae

    # 总结
    print(f"\n{'='*60}")
    print("总结")
    print(f"{'='*60}")
    print(f"\n  {'方法':<25} | {'500步MAE (rad)':>15}")
    print(f"  {'-'*45}")
    for name, mae in sorted(results.items(), key=lambda x: x[1]):
        print(f"  {name:<25} | {mae:15.5f}")
