"""Full GP+Ensemble+DAgger audit with fixed seeds.

Trains the actual GP+NN ensemble with DAgger and evaluates.
"""
import sys, os
...
def run_full_ensemble_audit():
    """Run full GP+Ensemble+DAgger evaluation."""
    print("=" * 70)
    print("FULL GP+ENSEMBLE+DAGGER AUDIT")
    print("=" * 70)
    ...
    for i in range(n_models):
        ...
        print(f"   Model {i} trained, final loss: {epoch_loss:.6f}")
    ...
    # 5. Run evaluation
    print("\n4. Running 500-step evaluations...")
    n_runs = 3
    n_segments = 5
    n_steps = 500
    ...
    # 6. Run with DAgger
    print("\n5. Running with DAgger (3 rounds)...")
    dagger_rounds = 3
    for round_i in range(dagger_rounds):
        ...
        for seg_i in range(3):
            tau_func = make_tau_func(100 + round_i * 3 + seg_i, n_steps)
...
if __name__ == '__main__':
    results = run_full_ensemble_audit()

    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'full_ensemble_audit_results.json')
