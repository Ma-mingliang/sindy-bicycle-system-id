"""Evaluate Long Curriculum model."""
import sys
import json
import numpy as np
import torch
...
def main():
    print("=" * 60)
    print("Evaluate Long Curriculum Model")
    print("=" * 60)

    data = load_data(seed=42)

    # Load model
    model_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/07_models/long_curriculum_50_seed43.pt'
    ...
    print("\n" + "=" * 60)
    print("RESULTS: Long Curriculum 50")
    print("=" * 60)
    ...
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Long Curric':<12} {'Improvement':<12}")
    print("-" * 46)
    v9_nmae = {1: 0.0051, 10: 0.0628, 50: 0.4557, 100: 0.5064, 200: 0.4737, 500: 0.5529}
    for h in horizons:
        v9_val = v9_nmae.get(h, float('nan'))
        lc_val = results[h]['nmae_mean']
        if v9_val > 0 and not np.isnan(lc_val):
            improvement = (v9_val - lc_val) / v9_val * 100
            print(f"H={h:<7} {v9_val:<12.4f} {lc_val:<12.4f} {improvement:<12.1f}%")

    # Save results
    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_72h',
        'experiment': 'long_curriculum_50',
        'config': config,
        'results': results,
    }
