"""Ablation Experiment for Constrained Neural ODE.

Tests the effect of different components:
1. Jacobian constraint strength
2. Residual scale
...
def main():
    print("=" * 60)
    print("Ablation Experiment for Constrained Neural ODE")
    print("=" * 60)
    ...
    ablation_configs = {
        ...
        'no_constraint': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
            'max_jacobian_norm': 1000.0, 'lambda_jacobian': 0.0,
        },
        'weak_constraint': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
        ...
        'strong_constraint': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
            'max_jacobian_norm': 0.01, 'lambda_jacobian': 10.0,
        },
        'larger_network': {
    ...
    output = {
        ...
        'experiment': 'ablation_constrained_neural_ode',
        'configs': list(ablation_configs.keys()),
        'results': {k: {'primary': v['primary']} for k, v in all_results.items()},
        'best_config': best_config,
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP019_ablation.json'
