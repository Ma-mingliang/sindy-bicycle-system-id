"""Test MPPI residual on top of LQR at v=3.5 m/s.

Compares:
  1. LQR only (baseline)
  2. LQR + MPPI residual (using SINDy world model)
...
# ============================================================
# SINDy World Model (linear, from identified coefficients)
# ============================================================

def sindy_predict_next_state(s, tau, A_d, B_d):
    """Predict next state using SINDy discrete-time model.
...
# ============================================================
# MPPI Residual Controller
# ============================================================

class MPPIResidual:
    """MPPI for computing residual on top of LQR.

    Uses SINDy world model to evaluate candidate residuals.
    """

    def __init__(self, A_d, B_d, K_lqr, target_theta,
                 n_samples=64, horizon=5, noise_std=0.05,
                 temperature=0.5, epsilon_max=0.15,
        ...
        # State reorder: LQR [phi,phidot,delta,deltadot] -> SINDy [phi,delta,phidot,deltadot]
        self.lqr_to_sindy = [0, 2, 1, 3]

    def compute_residual(self, state_lqr):
        """Compute MPPI residual.

        Args:
            state_lqr: [phi, phi_dot, delta, delta_dot]

        Returns: residual torque (scalar)
        """
        n_samples = self.n_samples
        horizon = self.horizon

        # Sample noise: (n_samples, horizon)
        ...
        for k in range(n_samples):
            ...
            for h in range(horizon):
                ...
                # Predict next state with SINDy
                s_sindy = s[self.lqr_to_sindy]
                s_next_sindy = sindy_predict_next_state(s_sindy, u_total,
                                                        self.A_d, self.B_d)
                # Convert back to LQR order
                s = s_next_sindy[[0, 2, 1, 3]]

                # Cost: tracking error + residual penalty + smoothness
                tracking_cost = (s[0] - tgt) ** 2
        ...
        # Weighted average of first-step noise
...
def run_episode(K_lqr, ep, seed, v0=3.5, use_mppi=False,
    ...
    if use_mppi and A_d is not None and B_d is not None:
        mppi = MPPIResidual(A_d, B_d, K_lqr, target_theta,
                            n_samples=64, horizon=5, noise_std=0.05,
                            temperature=0.5, epsilon_max=0.15)
    ...
    for step in range(max_steps):
        # Resample target_theta every 100 steps
        if step % 100 == 0:
            target_theta = np.clip(rng.normal(0, sigma), -math.pi / 12, math.pi / 12)
            if mppi is not None:
...
def main():
    print("=" * 70)
    print("MPPI Residual Test at v=3.5 m/s")
    print("=" * 70)
    ...
    # SINDy model (discrete-time)
    sindy_data = np.load('D:/系统辨识作业/sindy_bicycle/meijaard_sindy.npz')
    Xi = sindy_data['coefficients']
    A_lin = Xi[1:5, :].T
    B_lin = Xi[5:6, :].T
    dt_sindy = float(sindy_data['dt'])
    A_d = np.eye(4) + A_lin
    B_d = B_lin
    print(f"SINDy A_d eigenvalues: {np.linalg.eigvals(A_d)}")

    # Also load SINDy LQR for comparison
    K_sindy = np.load('D:/系统辨识作业/sindy_bicycle/sindy_lqr.npz')['K_lqr']

    num_seeds = 10

    print(f"\n{'='*70}")
    print(f"Progressive Disturbance Test (v={v0}, {num_seeds} seeds)")
    print(f"{'='*70}")
    ...
    for ep in [1, 10, 20, 30, 40, 53, 80]:
        ...
        for seed in range(num_seeds):
            ...
            lqr_fell += r['fell']
            lqr_steps.append(r['steps'])
            lqr_track.append(r['avg_track_err'])

            r = run_episode(K_true, ep, seed, v0, use_mppi=True,
                            A_d=A_d, B_d=B_d)
            mppi_fell += r['fell']
            mppi_steps.append(r['steps'])
            mppi_track.append(r['avg_track_err'])
            mppi_eps.append(r['avg_residual'])
    ...
    # === Also test with SINDy LQR ===
    print(f"\n{'='*70}")
    print(f"Same test with SINDy LQR (more aggressive gains)")
    print(f"{'='*70}")
    print(f"{'Ep':>4} {'sigma':>6} {'sig°':>5} | "
          f"{'LQR fell%':>9} {'steps':>6} {'track':>7} | "
          f"{'LQR+MPPI fell%':>14} {'steps':>6} {'track':>7} {'eps':>6} | "
    ...
    for ep in [1, 10, 20, 30, 40, 53, 80]:
        ...
        for seed in range(num_seeds):
            r = run_episode(K_sindy, ep, seed, v0, use_mppi=False)
            lqr_fell += r['fell']; lqr_steps.append(r['steps']); lqr_track.append(r['avg_track_err'])

            r = run_episode(K_sindy, ep, seed, v0, use_mppi=True, A_d=A_d, B_d=B_d)
