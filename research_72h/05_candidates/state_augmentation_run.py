"""State Space Augmentation Neural ODE - Quick Run Script."""
import sys
import os
import json
import time
...
def train_model(data, config, seed=43):
    ...
    print(f"  Training {variant} {augmented_dim}D (seed={seed})...")
    ...
    for epoch in range(config['n_epochs']):
        ...
        for sb, ab, yb in loader:
            ...
            if rollout_steps > 1 and len(sb) > rollout_steps + 1:
                ...
                for step in range(rollout_steps):
                    ...
                    loss_multi = loss_multi + nn.functional.mse_loss(x_next, target)
                    x_cur = x_next
                loss_multi = loss_multi / rollout_steps

            loss_jacobian = torch.tensor(0.0)
            if config.get('lambda_jacobian', 0) > 0 and epoch % 3 == 0:
                n_jac = min(16, len(sb))
                ...
                jac_norm = 0.0
                aug_d = z_req.shape[1]
                for i_dim in range(aug_d):
                    grad = torch.autograd.grad(dzdt[:, i_dim].sum(), s_req, create_graph=True)[0]
                    jac_norm = jac_norm + grad.pow(2).sum()
                loss_jacobian = jac_norm / (aug_d * n_jac)
        ...
        if (epoch + 1) % 50 == 0:
            ...
            print(f"    Epoch {epoch+1}: loss={avg_loss:.6f}, time={elapsed:.1f}s")
...
def main():
    ...
    print(f"\n{'Config':<25} {'H=1':<8} {'H=10':<8} {'H=50':<8} {'H=100':<8} {'H=200':<8} {'H=500':<8} {'Primary':<8}")
    print("-" * 83)
    ...
    for exp_name, data_dict in all_results.items():
        output['results'][exp_name] = {
            'config': {k: v for k, v in data_dict['config'].items()},
...
### Architecture Comparison: Direct vs Latent

- **Direct variant**: Observable states are passed through directly (x_obs in augmented state).
  The decoder adds a small correction. This preserves physical meaning and tends to be more stable.

- **Latent variant**: Full encoding into latent space. More flexible but harder to train.
  The decoder must learn to reconstruct the observable state entirely.

### Dimensionality Trade-offs

- **16D (9 hidden)**: Good balance between expressiveness and trainability.
- **32D (25 hidden)**: More capacity, but risks overfitting on limited data.

## Comparison with Other Methods

| Method | Primary Score | H=100 | H=500 |
|--------|---------------|-------|-------|
| v9 Neural ODE | - | {v9_baseline[100]:.4f} | {v9_baseline[500]:.4f} |
| State Augmentation (best) | {all_results[best_exp]['primary']:.4f} | {best_results[100]['nmae_mean']:.4f} | {best_results[500]['nmae_mean']:.4f} |
...
4. **Overfitting Risk**: More parameters may lead to overfitting on training data.
...
3. Investigate hidden state dynamics for physical insights (e.g., correlate with tire forces).
4. Test with Jacobian regularization to further control error growth.
5. For production use, combine with error correction or scheduled sampling.
"""
