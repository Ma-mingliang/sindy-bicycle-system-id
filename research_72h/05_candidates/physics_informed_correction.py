"""Physics-Informed State Correction.

Uses known bicycle dynamics physics to correct predictions.
Key insight: e_y_dot = v * sin(e_psi) is exact kinematics.
"""
import sys
import json
import time
...
def correct_state_physics(s_pred, s_cur, nn_model, train_obs_norm, state_std, dt,
    ...
    # Physics correction for e_y and e_psi
...
def evaluate_with_correction(model, data, state_std, action_std, delta_std, nn_model, train_obs_norm,
                              horizons, correction_strength=0.1, physics_weight=0.5, n_segments=5, seed=42):
    """Evaluate model with physics-informed state-space correction."""
    dt = 1.0 / 30.0
    episodes = data['episodes']
        ...
        for seg in segments:
            ...
            for step in range(n):
                try:
                    ...
                    # Apply physics-informed state-space correction
...
def main():
    print("=" * 60)
    print("Physics-Informed State Correction")
    print("=" * 60)
    ...
    physics_weights = [0.0, 0.3, 0.5, 0.7]
    ...
    for physics_weight in physics_weights:
        print(f"\n{'='*60}")
        print(f"Testing physics_weight={physics_weight}")
        print(f"{'='*60}")

        results = evaluate_with_correction(
            model, data, state_std, action_std, delta_std,
            nn_model, train_obs_norm, horizons,
            correction_strength=correction_strength, physics_weight=physics_weight
        )
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: Physics-Informed Correction Results")
    print("=" * 60)

    print(f"\n{'Physics':<12} {'H=50':<10} {'H=100':<10} {'H=200':<10} {'H=500':<10} {'Primary':<10}")
    print("-" * 62)

    for physics_weight, data_dict in all_results.items():
        r = data_dict['results']
        primary = data_dict['primary']
        print(f"{physics_weight:<12} {r[50]['nmae_mean']:<10.4f} {r[100]['nmae_mean']:<10.4f} "
              f"{r[200]['nmae_mean']:<10.4f} {r[500]['nmae_mean']:<10.4f} {primary:<10.4f}")

    # Find best configuration
    best_physics = min(all_results.keys(), key=lambda k: all_results[k]['primary'])
    print(f"\nBest physics weight: {best_physics} (Primary = {all_results[best_physics]['primary']:.4f})")
    ...
    best_results = all_results[best_physics]['results']
    for h in horizons:
    ...
    output = {
        ...
        'experiment': 'physics_informed_correction',
    ...
    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP032_physics_informed_correction.json'
