"""Improved Coupling Network for Per-State Prediction.

Improvements over basic Coupling Network:
1. Larger coupling dimension (128, 256) - reduce bottleneck
2. Decoder interaction - decoders can see other states
3. Residual connections - direct access to original states
4. Multi-step training - train with rollout

Expected: Better than Wide SiLU NODE (0.3930)
"""
import sys
import json
import time
...
class ImprovedCouplingEncoder(nn.Module):
    """Improved shared encoder with residual connections."""
    def __init__(self, input_dim, coupling_dim=128, hidden=128):
        super().__init__()
...
class ImprovedStateDecoder(nn.Module):
    """Improved state decoder with access to all states."""
    def __init__(self, coupling_dim, hidden=64):
        super().__init__()
        # Takes coupling vector + all states → predicts delta for one state
        self.net = nn.Sequential(
            nn.Linear(coupling_dim + STATE_DIM, hidden),  # +7 for all states
            nn.SiLU(),
            nn.Linear(hidden, hidden),
            nn.SiLU(),
            nn.Linear(hidden, 1),
    ...
    def forward(self, z, all_states):
        """Input: coupling vector z + all states → Output: delta for this state."""
        x = torch.cat([z, all_states], dim=-1)
        return self.net(x).squeeze(-1)


class ImprovedCouplingNetwork(nn.Module):
    """Improved Coupling Network.

    Improvements:
    1. Larger coupling dimension (128) - reduce bottleneck
    2. Decoder interaction - decoders see all states
    3. Residual connections - direct access to original states
    """
    def __init__(self, state_dim=7, action_dim=1, coupling_dim=128, hidden=128):
        super().__init__()

        # Improved encoder with residual
        ...
        # Improved decoders with access to all states
    ...
    def forward(self, s, a):
        ...
        # State-specific decoders with access to all states
        deltas = []
        for i, decoder in enumerate(self.decoders):
            delta_i = decoder(z, s)  # Pass all states, not just own state
...
def train_improved_coupling(data, config, seed=42):
    """Train improved coupling network."""
    ...
    print(f"Training Improved Coupling Network (seed={seed})...")
...
def main():
    print("=" * 60)
    print("Improved Coupling Network")
    print("=" * 60)

    # Test different configurations
    configs = [
        {'name': 'large_coupling', 'coupling_dim': 128, 'hidden': 128, 'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256},
        {'name': 'xlarge_coupling', 'coupling_dim': 256, 'hidden': 256, 'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256},
    ]
    ...
    for config in configs:
        ...
        model, state_std, action_std, delta_std = train_improved_coupling(data, config, seed=42)
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: Improved Coupling Network Results")
    print("=" * 60)
    ...
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Wide SiLU':<12} {'Best Improved':<12}")
    print("-" * 46)
    ...
    for h in horizons:
        ...
        ic_val = best_results[h]['nmae_mean']
        print(f"H={h:<7} {v9_val:<12.4f} {ws_val:<12.4f} {ic_val:<12.4f}")

    # Save results
    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_144h',
        'experiment': 'improved_coupling_network',
        'configs': [c['name'] for c in configs],
        'results': {k: {'primary': v['primary']} for k, v in all_results.items()},
        'best_config': best_name,
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP051_improved_coupling.json'
