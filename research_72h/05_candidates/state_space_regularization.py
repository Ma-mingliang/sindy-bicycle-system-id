"""State Space Regularization - New Direction.

Key insight: Model is good at short-term but loses accuracy at long-term.
Solution: Constrain predictions to stay within training distribution.
"""
import sys
import json
import time
...
def correct_state(s_pred, nn_model, train_obs_norm, state_std, correction_strength=0.5):
...
def evaluate_with_correction(model, data, state_std, action_std, delta_std, nn_model, train_obs_norm,
                              horizons, correction_strength=0.5, n_segments=5, seed=42):
    """Evaluate model with state-space correction."""
    dt = 1.0 / 30.0
    episodes = data['episodes']
...
def main():
    print("=" * 60)
    print("State Space Regularization")
    print("=" * 60)
    ...
    correction_strengths = [0.0, 0.1, 0.3, 0.5, 0.7]
    ...
    for strength in correction_strengths:
        print(f"\n{'='*60}")
        print(f"Testing correction_strength={strength}")
        print(f"{'='*60}")

        results = evaluate_with_correction(
            model, data, state_std, action_std, delta_std,
            nn_model, train_obs_norm, horizons, correction_strength=strength
        )
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: State Space Regularization Results")
    print("=" * 60)
    ...
    print(f"\nBest correction strength: {best_strength} (Primary = {all_results[best_strength]['primary']:.4f})")

    # Compare with v9 baseline
    print(f"\nComparison with v9 baseline:")
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Best Corrected':<12} {'Improvement':<12}")
    print("-" * 46)
    ...
    output = {
        ...
        'experiment': 'state_space_regularization',
        'correction_strengths': correction_strengths,
        'results': {str(k): {'primary': v['primary']} for k, v in all_results.items()},
        'best_strength': best_strength,
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP028_state_space_regularization.json'
