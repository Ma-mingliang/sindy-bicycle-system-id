"""MBPO-SAC v2: HRRL-aligned reward + warmup phase.

Key improvements over v1:
1. Delta-based reward from HRRL (env.py:190-202)
   - |dis| < 0.002: reward = 0.1 - |w0|
   - else: reward = |prev_dis| - |new_dis|
   - terminated: -1 - 0.01*|w0|
2. Warmup phase: first 100 steps target_theta=0, k=0 (env.py:260-264)
3. More virtual rollouts (10x8) for better model usage
4. 600 episodes for longer training
"""
...
# ============================================================
# Environment with HRRL-style reward
# ============================================================
...
def train():
    print("=" * 70)
    print("MBPO-SAC v2: HRRL-aligned reward + warmup")
    print("=" * 70)
    ...
    env = MeijaardAttitudeEnv(K_true, v0=v0, max_steps=1000)
    ...
    real_buffer = ReplayBuffer(200000)
    virtual_buffer = ReplayBuffer(200000)
    ...
    print(f"Warmup: first 100 steps target_theta=0")
    ...
    }, 'D:/系统辨识作业/sindy_bicycle/mbpo_sac_v2.pt')
    print("\nModel saved to mbpo_sac_v2.pt")

    # Evaluation
    print(f"\n{'='*70}")
    print("Evaluation: LQR vs LQR+MBPO-SAC v2")
    print(f"{'='*70}")
