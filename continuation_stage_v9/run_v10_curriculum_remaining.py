"""V10 Experiment 2: Remaining curriculum configs."""
import sys
...
def load_existing():
    path = os.path.join(RESULTS_PATH, 'V10_CURRICULUM.json')
...
def run_experiment(data, segments, state_std, name, config):
    ...
    result = {
        'config': {
            ...
            'n_epochs': config.n_epochs, 'seed': config.seed,
        },
...
def main():
    print("=" * 70)
    print("V10 Experiment 2: Remaining Curriculum Configs")
    print("=" * 70)
    t_start = time.time()
    ...
    print(f"\n[1] Loaded {len(results)} existing results")
