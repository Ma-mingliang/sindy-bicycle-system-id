"""Noise-Augmented Training - Address Root Cause.

Root cause: Model never learns to recover from its own errors.
Solution: Add noise to initial states during training.
"""
import sys
import json
import time
...
def train_noise_augmented(data, config, seed=43):
    """Train Neural ODE with noise-augmented initial states."""
    ...
    print(f"Training Noise-Augmented Neural ODE (seed={seed})...")
    ...
    for epoch in range(config['n_epochs']):
        ...
        for sb, ab, yb in loader:
            ...
            noise = torch.randn_like(sb) * noise_scale
            sb_noisy = sb + noise

            pred = model(sb_noisy, ab)
...
def main():
    print("=" * 60)
    print("Noise-Augmented Training - Address Root Cause")
    print("=" * 60)
    ...
    for name, config in noise_configs.items():
        print(f"\n{'='*60}")
        print(f"Testing: {name} (noise_scale={config['noise_scale']})")
        print(f"{'='*60}")

        model, state_std, action_std, delta_std = train_noise_augmented(data, config, seed=43)
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: Noise-Augmented Training Results")
    print("=" * 60)
    ...
    print(f"{'Horizon':<10} {'v9 NMAE':<12} {'Best Noise':<12} {'Improvement':<12}")
    print("-" * 46)
    ...
    for h in horizons:
        ...
        n_val = best_results[h]['nmae_mean']
        if v9_val > 0 and not np.isnan(n_val):
            improvement = (v9_val - n_val) / v9_val * 100
            print(f"H={h:<7} {v9_val:<12.4f} {n_val:<12.4f} {improvement:<12.1f}%")

    # Save results
    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_144h',
        'experiment': 'noise_augmented_training',
        'configs': list(noise_configs.keys()),
        'results': {k: {'primary': v['primary']} for k, v in all_results.items()},
        'best_config': best_config,
        'root_cause': 'training-test distribution mismatch',
        'solution': 'noise augmentation to simulate prediction errors',
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP025_noise_augmented.json'
