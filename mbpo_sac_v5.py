"""MBPO-SAC v5: SAC as DIRECT controller (not residual).

Previous versions tried RL residual on top of LQR, but LQR is
near-optimal for the linear Meijaard model so RL can't improve it.

v5 approach:
1. SAC outputs FULL control torque (not residual)
2. Reward encourages matching LQR performance
3. Progressive disturbance training
4. MBPO with SINDy world model
"""
...
class SINDyWorldModel:
    ...
    def predict_next(self, theta, delta, theta_dot, delta_dot, tau):
...
class DirectControlEnv:
    """SAC outputs FULL torque, no LQR base."""

    def __init__(self, K_lqr, v0=3.5, dt=1/30, max_steps=1000, tau_scale=50.0):
        self.K_lqr = K_lqr
        self.v0 = v0
        self.dt = dt
        self.max_steps = max_steps
        self.g = 9.81
        self.tau_scale = tau_scale  # Scale SAC output to reasonable torque range
        p = {
    ...
    def step(self, action):
        ...
        # SAC outputs full torque (scaled)
        tau = action_val * self.tau_scale
...
def sindy_virtual_rollout(model, buffer, agent, tau_scale, n_rollouts=10, horizon=8):
    data = []
    for _ in range(n_rollouts):
        ...
        for h in range(horizon):
            ...
            tau = float(action[0]) * tau_scale
            tn, dn, wn, vn = model.predict_next(theta, 0.0, w, v, tau)
            new_dis = target - tn
...
def train():
    print("=" * 70)
    print("MBPO-SAC v5: Direct control (no LQR base)")
    print("=" * 70)
    ...
    # tau_scale: LQR typical torque is ~50-200, SAC output is [-1,1]
    # Use tau_scale so SAC can reach similar torque magnitudes
    tau_scale = 100.0
    print(f"Torque scale: {tau_scale}")
    ...
    env = DirectControlEnv(K_lqr, v0=v0, max_steps=1000, tau_scale=tau_scale)
    ...
    print(f"Mode: SAC direct control (no LQR base)")
    print(f"{'Ep':>4} {'sigma':>6} {'sig°':>5} | {'Reward':>8} {'Steps':>6} "
          f"{'Track':>7} {'Tau':>6} | {'Alpha':>6} {'Buf':>6}")
    ...
    for ep in range(num_episodes):
        ...
        while True:
            ...
            ep_tau += abs(action[0]) * tau_scale
            steps += 1
            state = next_state
            if steps % 10 == 0 and len(real_buffer) >= batch_size:
                vdata = sindy_virtual_rollout(sindy_model, real_buffer, sac,
                                              tau_scale, 10, 8)
    ...
    }, 'D:/系统辨识作业/sindy_bicycle/mbpo_sac_v5.pt')
    print("\nModel saved to mbpo_sac_v5.pt")

    # Evaluation: LQR vs SAC direct control
    print(f"\n{'='*70}")
    print("Evaluation: LQR vs SAC Direct Control v5")
    print(f"{'='*70}")
    print(f"{'Ep':>4} {'sig°':>5} | {'LQR fell%':>9} {'steps':>6} {'track':>7} | "
          f"{'SAC fell%':>9} {'steps':>6} {'track':>7} | {'Gap':>8}")
    ...
    for ep in [1, 10, 20, 30, 40, 53, 80]:
        ...
        for seed in range(n):
            ...
            total = 0.0; fell = False
            for step in range(1000):
                obs, _, done, info = env.step_lqr()
                total += info['tracking_err']
                state = obs
                if done:
                    fell = abs(env.theta) > math.pi / 3
                    break
            lqr_fell += fell; lqr_steps.append(step + 1); lqr_track.append(total / (step + 1))
            ...
            for step in range(1000):
                ...
                if done:
                    fell = abs(env.theta) > math.pi / 3
                    break
            sac_fell += fell; sac_steps.append(step + 1); sac_track.append(total / (step + 1))

        gap = (np.mean(sac_track) / (np.mean(lqr_track) + 1e-10) - 1) * 100
        print(f"{ep:4d} {math.degrees(sigma):4.1f}° | "
