"""
Version T: Trajectory-Synchronization Residual DAgger Implementation

Residual formula:
    r_k = (s_real[k+1] - GP(s_model[k], u_k)) / delta_std

Real dynamics evaluates at s_real[k] to produce s_real[k+1].
GP evaluates at s_model[k].
The residual pulls the model trajectory toward the real trajectory.

Key difference from Version L:
- Version T: s_real[k+1] vs GP(s_model[k], u_k) -> different trajectories
- Version L: F_real(s_model[k], u_k) vs GP(s_model[k], u_k) -> same state
"""
...
class VersionTTrajectoryResidual:
    """
    GP + NN ensemble with Version T (Trajectory Synchronization Residual) DAgger.
    ...
    The residual is the next-state gap between real and model trajectories.

    This is the simplest and most standard trajectory-level DAgger residual.
    """
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, deltas: np.ndarray,
        ...
        """Train GP baseline + NN ensemble with Version T DAgger."""
        ...
        # 2. Round 0: Use Version T residual at expert states
        # For consistency, Round 0 also uses trajectory-sync: s_real[k+1] vs GP(s_k, u_k)
        # Note: at expert states, s_model = s_real, so Version T and Version L are equivalent
        residuals = np.empty_like(deltas)
        for i in range(len(states)):
            s_next_base = self._baseline.predict(states[i], actions[i])
            s_next_real = states[i] + deltas[i]  # = real next state
            # Version T: gap between real next state and GP next state
            residuals[i] = (s_next_real - s_next_base) / delta_std

        # 3. DAgger rounds with Version T residual
        ...
        for round_i in range(self.dagger_rounds):
            ...
            # DAgger: collect new data with Version T residual
            if round_i < self.dagger_rounds - 1:
                new_inputs, new_residuals = self._collect_dagger_data_vT(
    ...
    def _collect_dagger_data_vT(self, models, state_std, action_std, delta_std,
                                 make_tau_func, real_step):
        """
        Collect DAgger data with Version T residual.

        At each step k:
            1. Compute tau from real state (standard DAgger)
            2. Compute real dynamics: s_real[k+1] = F_real(s_real[k], u_k)
            3. Compute GP prediction: s_next_gp = GP(s_model[k], u_k)
            4. Residual = (s_real[k+1] - s_next_gp) / delta_std
            5. Update model state: s_model[k+1] = s_next_gp + scaled_NN(s_model[k], u_k)

        The residual is the trajectory gap: how far the real next state
        is from where the model predicts it should be.
        """
        new_inputs = []
        new_residuals = []
        for seg_i in range(3):
            ...
            for step in range(500):
                ...
                # Advance REAL trajectory
                s_real_next = real_step(s_real, tau)

                if abs(s_real_next[0]) > math.pi / 3:
                    break
                ...
                # Residual: trajectory gap between real next state and GP next state
                # Note: s_real_next is at s_real[k], s_next_gp is at s_model[k]
                ...
                # Advance real trajectory reference for next step
                s_real = s_real_next
    ...
    def name(self) -> str:
        return f'version_T_{self.n_models}m_{self.dagger_rounds}r'
