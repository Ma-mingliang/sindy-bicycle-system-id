...
Addresses OOD robustness by training with OOD-like data augmentation.
Key idea: Augment training data with OOD-like perturbations.
"""
import sys
import json
import time
...
def train_ood_robust(data, config, seed=42):
    """Train Neural ODE with OOD robust data augmentation."""
    ...
    # OOD augmentation parameters
    ood_noise_scale = config.get('ood_noise_scale', 0.1)
    ood_prob = config.get('ood_prob', 0.3)

    print(f"Training OOD robust model (seed={seed})...")
    print(f"  ood_noise_scale: {ood_noise_scale}")
    print(f"  ood_prob: {ood_prob}")
    ...
    for epoch in range(config['n_epochs']):
        ...
        for sb, ab, yb in loader:
            # OOD augmentation: add noise to simulate OOD states
            if np.random.random() < ood_prob:
                # Add noise to simulate OOD states
                noise = torch.randn_like(sb) * ood_noise_scale
                sb_aug = sb + noise

                # Also augment actions
                action_noise = torch.randn_like(ab) * ood_noise_scale * 0.5
                ab_aug = ab + action_noise

                # Train on augmented data
                pred = model(sb_aug, ab_aug)
...
def evaluate_ood(model, ood_scenarios, state_std, action_std, delta_std, nn_model, train_obs_norm,
                  horizons, correction_strength=0.1, physics_weight=0.7):
    """Evaluate model on OOD scenarios with physics-informed correction."""
    dt = 1.0 / 30.0
...
def main():
    print("=" * 60)
    print("OOD Robust Training")
    print("=" * 60)

    # Test different OOD augmentation configurations
    ood_configs = {
        'no_ood_augmentation': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
            'ood_noise_scale': 0.0, 'ood_prob': 0.0,
        },
        'mild_ood_augmentation': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
            'ood_noise_scale': 0.05, 'ood_prob': 0.2,
        },
        'moderate_ood_augmentation': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
            'ood_noise_scale': 0.1, 'ood_prob': 0.3,
        },
        'aggressive_ood_augmentation': {
            'hidden': 64, 'depth': 3, 'activation': 'tanh',
            'lr': 1e-3, 'n_epochs': 200, 'batch_size': 256,
            'ood_noise_scale': 0.2, 'ood_prob': 0.5,
        },
    }
    ...
    for name, config in ood_configs.items():
        print(f"\n{'='*60}")
        print(f"Testing: {name}")
        print(f"{'='*60}")

        model, state_std, action_std, delta_std = train_ood_robust(data, config, seed=42)

        print(f"\nGenerating OOD scenarios...")
        ood_scenarios = generate_ood_scenarios(data, state_std)

        print(f"\nEvaluating on OOD scenarios...")
        ood_results = evaluate_ood(model, ood_scenarios, state_std, action_std, delta_std,
                                    nn_model, train_obs_norm, horizons)
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: OOD Robust Training Results")
    print("=" * 60)

    print(f"\n{'Config':<30} {'H=50':<10} {'H=100':<10} {'H=200':<10} {'H=500':<10}")
    print("-" * 70)

    for name, data_dict in all_results.items():
        avg = data_dict['avg_ood_nmae']
        print(f"{name:<30} {avg[50]:<10.4f} {avg[100]:<10.4f} {avg[200]:<10.4f} {avg[500]:<10.4f}")

    # Save results
    output = {
        'timestamp': datetime.now().isoformat(),
        'run_id': '20260628_175824_neural_ode_144h',
        'experiment': 'ood_robust_training',
        'configs': list(ood_configs.keys()),
        'results': {k: {'avg_ood_nmae': {str(h): v for h, v in data_dict['avg_ood_nmae'].items()}}
                   for k, data_dict in all_results.items()},
    }

    output_path = 'D:/系统辨识作业/sindy_bicycle/research_72h/05_candidates/EXP036_ood_robust_training.json'
