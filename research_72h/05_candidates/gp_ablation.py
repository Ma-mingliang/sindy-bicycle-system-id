...
def train_gp_model(data, kernel, n_samples=5000, seed=42):
    """Train GP model."""
    np.random.seed(seed)
...
def main():
    ...
    for name, config in ablation_configs.items():
        print(f"\n{'='*60}")
        print(f"Ablation: {name} - {config['description']}")
        print(f"{'='*60}")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("SUMMARY: GP Ablation Results")
    print("=" * 60)

    print(f"\n{'Config':<20} {'H=50':<10} {'H=100':<10} {'H=200':<10} {'H=500':<10} {'Primary':<10} {'Time':<10}")
    print("-" * 80)
    ...
    for name, data_dict in all_results.items():
        ...
        print(f"{name:<20} {r[50]['nmae_mean']:<10.4f} {r[100]['nmae_mean']:<10.4f} "
              f"{r[200]['nmae_mean']:<10.4f} {r[500]['nmae_mean']:<10.4f} {primary:<10.4f} {train_time:<10.1f}")
    ...
    output = {
        ...
        'results': {k: {'primary': v['primary'], 'train_time': v['train_time'], 'description': v['description']}
